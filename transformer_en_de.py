...
class MultiheadAttention(nn.Module):
  ...
  def forward(self, query, key, value, mask = None):
    ...
    multi_concat = torch.cat([head(query, key, value, mask) for head in self.multiheads],dim=-1)
...
class EncoderBlock(nn.Module):
  ...
  def forward(self, x, mask=None):
    ...
    att = self.heads(query=x, key=x, value=x, mask=mask)
...
MAX_LEN = 100
train_data, val_data, test_data = datasets.IWSLT.splits(exts=('.de', '.en'), fields=(SRC, TGT), 
...
# Train Epoch
def train_epoch(train_iter, model, criterion, opt, epoch, writer, SRC=SRC, TGT=TGT):
    model.train()
    best_loss = 1.0
    for i, batch in enumerate(train_iter):
        ...
        if epoch%10==0 and epoch>0:
          ...
          torch.save(model.state_dict(), "./model/saved_epoch%d.pt"%(epoch))             
        model_opt.step()
...
# Validation epoch
def valid_epoch(valid_iter, model, criterion, writer, SRC=SRC, TGT=TGT):
    model.eval()
    total = 0
...
model = EncoderDecoder(tgt_vocab=len(TGT.vocab), src_vocab=len(SRC.vocab), d_model=512, d_out=64, d_ff=1024, 
                       n_heads=8, n_blocks=6, dropout=0.1, max_len=3000)
...
for epoch in range(30):
    print('============== Epoch ',epoch,' ==============')
    train_epoch((rebatch(pad_idx, b) for b in train_iter), model, criterion, model_opt, epoch, writer)
